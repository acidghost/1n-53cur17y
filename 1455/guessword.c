...
#define HASHT_SIZE MAX_HASH - SUBT_HASH
...
int main(int argc, char const *argv[]) {
  ...
  static char *hash_table[HASHT_SIZE];
  while (!feof(dict_file) && fgets(dict_line, MAX_LINE - 1, dict_file)) {
    remove_newline(dict_line);
    char *encrypted = crypt(dict_line, first_uh->salt);
    unsigned long h = hash(encrypted) - SUBT_HASH;
    hash_table[h] = (char *) malloc(MAX_LINE * sizeof(char));
    strcpy(hash_table[h], dict_line);
  }
  ...
  while (!feof(pwd_hash_file) && fgets(pwd_line, MAX_LINE - 1, pwd_hash_file)) {
    ...
    unsigned long h = hash(uh->hash) - SUBT_HASH;
    if (hash_table[h]) {
